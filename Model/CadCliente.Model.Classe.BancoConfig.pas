...
interface
  ...
  type
    TBancoConfig = class
      //funções para setar configuração de banco de dados
    public
...
procedure TBancoConfig.CriarIni(Nome, Local: string);
  ...
  IniNome: String;
begin
  //Parametro para ciração do arquivo ini local (no caso o local
...
begin
    //Chamando a função de criação base.ini caso ele não exista
  if not FileExists(ExtractFilePath(ParamStr(0)) + 'base.ini') then
  begin
    showmessage('Não existe Banco de Dados configurado!');
